function output = evalGalPathway_GAL34_changedR( param, trait, G2level, G3level, G4level, fit_type )
% this function is updated from 'evalGalPathway_GAL234'
% it's called by 'mcmc_for_GAL234_prior_included' and 'param_fitting_plot_GAL234'
...
% GAL1_obj = calculate_obj_average_off( trait, G1basal, G1induc );
...
if G3level == 0     % do not fit G3
    ...
    % G3basal = y_ss_Glu(:,3);
    % G3induc = y_ss_Gal(:,3);
    G3basal = y_ss_Glu(:,3) + y_ss_Glu(:,6) + y_ss_Glu(:,8);    % G3, G3*, C83
    G3induc = y_ss_Gal(:,3) + y_ss_Gal(:,6) + y_ss_Gal(:,8);
...
if G4level == 0     % do not fit G4
    ...
    G4basal = y_ss_Glu(:,4) + y_ss_Glu(:,9);       % G4, C84
    G4induc = y_ss_Gal(:,4) + y_ss_Gal(:,9);
